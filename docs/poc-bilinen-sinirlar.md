...
- **LRU tavanıyla etkileşim.** Cache **orijinali** tutar (proxy'yi değil). Tavan 20 GiB
  ...
  indirmeyi baştan öder. Süpürmenin kendisi bu turda **ÖLÇÜLMEDİ** (cache 20 GiB'e
  doldurulmadı); ölçülen şey tavanın **%7,5**'inin tek bir 1,5 GiB'lık dosyayla dolduğudur.
- **Cache ölçümü ZAMAN DAMGALIDIR — çünkü BİRİM TESTLERİ bu dizini siliyor (ölçüldü).**
...
- **Disk rezervasyonu tahmini bu rejimde KISA KALIYOR (ölçülmüş bulgu).**
  `ExportJob.EstimateRequiredDiskBytes` çıktı boyutunu profil başına **10 Mbps** varsayımıyla
  hesaplıyor (`ExportProfiles.EstimatedBitsPerSecond`): 10:40'lık iş için
  (1 618 733 974 + 800 000 000) × 1,2 = **2 902 480 768 B** "gerekli" dedi; ölçülen tepe
  kullanım **3 926 837 249 B** oldu — yani gerçek ihtiyaç tahminin **1,35 katı**. Bu makinede
  ~390 GB boş alan vardı, kapı hiç ısırmadı; **dar diskli bir kurulumda** kapı "yeter" deyip
  render ortasında disk bitebilir. Kayıt: `docs/backlog.md` 12. tur.
...
bu boyutta **13 dosya** sığıyor. **Kota YALNIZ orijinalleri sayar** (`UploadQuota.Evaluate`
`Assets.SizeBytes` toplamını okur): türevler (+%7,7) sayılmaz, yani "20 GiB kota" gerçekte
~21,5 GiB'lik nesne deposu demektir.
...
> `test.fail` işaretlerinin **tam 7** olduğu ayrıca sayıldı.
>
