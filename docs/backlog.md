...
- **409 sonrası UX**: "başka sekmede değişti" diyaloğu + SignalR ile pasif sekme bilgilendirme (plandaki bilinçli MVP kabulü). *14. tur triyaj ölçümü (2026-08-21, BG bulgu B6):* asset listesi poll'u yalnız listede uploaded/processing satır varken koşuyor (`entities/assets.ts` refetchInterval); başka istemcinin/sekmenin yüklediği asset ve kota göstergesi, odak/yenileme olmadan görünmüyor (canlıda yeniden ölçüldü: API'den yüklenen iki asset ready olduktan sonra açık sekme 192 medyada kaldı, yenilemeyle 194 geldi). Kod içinde "SignalR gelene kadar kabul edilmiş ara çözüm" beyanı doğru — SignalR maddesi kapsamına bu ölçüm dahildir. *(2026-08-31 düzeltme: SignalR ilerleme kanalı GELDİ ama bu ölçümü KAPATMAZ — abonelik id-bazlıdır (`job:{id}`/`asset:{id}`, sekmenin ZATEN listelediği satırlar), kullanıcı-akışı grubu (`user:{id}` — "listen yeni satır doğdu" yayını) bilinçli kapsam dışı kaldı. B6 (pasif sekme/çapraz istemci görünürlüğü) AÇIK; kapatmak user-feed grubu ister.)* *(2026-08-31 KAPANIŞ — yarim-is-2 #5, user-feed grubu geldi: worker'ın her progress publish'i artık `ownerId` (Jobs.RequestedBy) taşır ve forwarder mesajı sahibinin `user:{id}` grubuna DA yollar; abonelik PARAMETRESİZ `SubscribeUserFeed` (kimlik JWT'den — başkasının feed'i hub yüzeyinde adreslenemez, IDOR aynası `CrossUserAccessTests.Hub_SubscribeUserFeed_*`); kitaplığı açık sekme feed'e abonedir ve BİLMEDİĞİ assetId'de listeyi+kotayı BİR kez invalidate eder (bilinen id'ler mevcut `asset:{id}` yolunda — fırtına yok, canlı koşumda upload→Hazır toplam 2 liste GET'i). Önce ölçüldü: kusur `e2e/library-crosstab-sync.spec.ts` ile HEAD'de yeniden üretildi (pasif sekme 30 sn'de satırı görmedi); publish gecikmesi n=3 complete→ilk publish 0,62-1,79 s (p50 0,74) → worker-publish yeterli, API'ye yeni Redis publisher yönü AÇILMADI (DECISIONS satırı). Kapanış kanıtı aynı spec'in yeşili: satır 2,85 sn'de + kota kendiliğinden, sayfa yüklemesi 0, ilk liste GET'i ilk feed mesajından SONRA. Yedek yol değişmedi (hub yoksa SignalR-öncesi davranış — progress-fallback.spec yeşil). 409 diyaloğu bu kaydın AYRI yarısıdır ve zaten kapalıydı (çakışma diyaloğu + K1 birim pini); kayıt KAPANDI.)* *(2026-08-31 ek — gelistirme-3 #2a: bu kapanışın SİLME yarısı ayrıca teslim edildi. SoftDelete hiçbir yayın tetiklemiyordu (worker işi doğmaz → feed'e mesaj düşmez); pasif sekme silinen asseti süresiz görüyordu — `e2e/library-crosstab-delete.spec.ts` HEAD'de KIRMIZI üretildi. API artık soft-delete sonrası sahibinin feed grubuna süreç-içi `assetRemoved` (`AssetRemovedMessage`, Contracts tek tanım) yollar; istemci satırı cache cerrahisiyle düşürür + yalnız kotayı invalidate eder. Spec yeşili: satır 33 ms, silme sonrası liste GET 0. DECISIONS "Silme senkronu (2a)" satırı.)*
...
| Revision retention job | ✅ **KAPANDI (2026-08-31, gelistirme-3 #2b)**: `ProjectRevisionRetentionJob` (saatlik recurring `revision-retention`, AssetReaperJob deseni) — proje başına son 50 Auto + 24 sa'ten eskilerde saatte 1'e inceltme; Checkpoint/PreRestore dokunulmaz (silme sorgusunda da Kind kilidi). Sabitler `RevisionRetention__*` config'inden (açılışta Validate + etkin-değer logu). Canlı ölçüm: dev DB'de 15 861 Auto birikmişti; tetiklenen iş seed'li 60 eskiden 10'unu süpürdü, korumalı türler kaldı. Muhafız: `RevisionRetentionJobTests.WorkerProgram_RegistersBothRecurringJobs` (Program.cs kaynak taraması). DECISIONS "Revision retention (2b)" satırı (tarif yorumu + reddedilenler) |
...
- **Revision retention job**: plandaki "son 50 auto + eskilerde inceltme" (denetim #5).
  ✅ **KAPANDI (2026-08-31, gelistirme-3 #2b)** — yukarıdaki tablo satırında ayrıntı.
